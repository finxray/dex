...
contract QuoterCtxDecode is IQuoter {
    function quote(QuoteParams memory params, bytes memory routed) external pure returns (uint256) {
        ...
        // Decode d3 as (bytes d3raw, bytes contextBytes) and ignore outputs
        ...
        if (contextBytes.length > 0) {
            ...
            // no emit; just proving decoding happens
            (ctx.trader); // silence warnings
        }
    ...
    function quoteBatch(QuoteParamsBatch memory params, bytes memory routed) external pure returns (uint256[] memory quotes) {
        (bytes memory d0, bytes memory d1, bytes memory d2, bytes memory d3, bytes memory dx) = abi.decode(routed, (bytes, bytes, bytes, bytes, bytes));
        (bytes memory d3raw, bytes memory contextBytes) = abi.decode(d3, (bytes, bytes));
        if (contextBytes.length > 0) {
            TraderContext memory ctx = abi.decode(contextBytes, (TraderContext));
