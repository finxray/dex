...
describe("UNI/USDC Bridge Tests - RedStone & Chainlink (fork)", function () {
  ...
  before(async () => {
    ...
    console.log("🔄 Using recent mainnet fork for up-to-date UNI/USDC price data");
  });
  ...
  it("UniswapV2DataBridge UNI/USDC spot", async function () {
    const [signer] = await ethers.getSigners();
    const Bridge = await ethers.getContractFactory("UniswapV2DataBridge");
    const bridge = await Bridge.deploy(UNI_V2_FACTORY, USDC, UNI, USDC, UNI);
    const Measurer = await ethers.getContractFactory("GasMeasurer");
    const meas = await Measurer.deploy();
    const p = { asset0: UNI, asset1: USDC, quoter: ethers.ZeroAddress, amount: 0, asset0Balance: 0, asset1Balance: 0, bucketID: 0, zeroForOne: true };
    const { execGas, spot } = await estimateAndDecode(meas, bridge, p, signer);
    console.log("UniswapV2 UNI/USDC execGas:", execGas.toString(), fmtBoth(spot, "UNI", "USDC"));
    expect(execGas).to.be.gt(0n);
  });

  it("UniswapV3DataBridge UNI/USDC spot", async function () {
    const [signer] = await ethers.getSigners();
    const Bridge = await ethers.getContractFactory("UniswapV3DataBridge");
    const bridge = await Bridge.deploy(UNI_V3_FACTORY, UNI_V3_FEE, 10800, USDC, UNI, USDC, UNI);
    const Measurer = await ethers.getContractFactory("GasMeasurer");
    const meas = await Measurer.deploy();
    const p = { asset0: UNI, asset1: USDC, quoter: ethers.ZeroAddress, amount: 0, asset0Balance: 0, asset1Balance: 0, bucketID: 0, zeroForOne: true };
    const { execGas, spot } = await estimateAndDecode(meas, bridge, p, signer);
    console.log("UniswapV3 UNI/USDC execGas:", execGas.toString(), fmtBoth(spot, "UNI", "USDC"));
    expect(execGas).to.be.gt(0n);
  });

  it("ChainlinkDataBridge UNI/USDC spot/updatedAt (Feed Registry)", async function () {
    ...
    try {
      ...
      console.log("Chainlink(Registry) UNI/USDC execGas:", execGas.toString(), fmtBoth(spot, "UNI", "USDC"), "updatedAt=", `${updatedAt} (${iso})`, "delta(s:m:h)=", `${diffSec}:${diffMin}:${diffHr}`);
      expect(execGas).to.be.gt(0n);
    } catch (error) {
  ...
  it("RedstoneOnDemandBridge UNI/USDC live (primary-prod)", async function () {
    ...
    const dataFeeds = ["UNI", "USDC"]; // UNI/USDC
    console.log("DEBUG: Requesting RedStone feeds (prod) for UNI/USDC:", dataFeeds);
    const authorised = [
    ...
    try {
      ...
      console.log("Redstone(on-demand prod) UNI/USDC spot:", fmtBoth(spot, "UNI", "USDC"), "updatedAt=", `${updatedAt} (${iso})`);
  ...
  it("RedstoneOnDemandBridge UNI/USDC live (primary-demo)", async function () {
    const { DataServiceWrapper } = require("@redstone-finance/evm-connector");
    ...
    const dataFeeds = ["UNI", "USDC"]; // UNI/USDC
    console.log("DEBUG: Requesting RedStone feeds (demo) for UNI/USDC:", dataFeeds);
    const demoSigners = [
    ...
    try {
      ...
      const ms = Number(updatedAt);
      const iso = Number.isFinite(ms) && ms > 0 ? new Date(ms).toISOString() : "n/a";
      console.log("Redstone(on-demand demo) UNI/USDC spot:", fmtBoth(spot, "UNI", "USDC"), "updatedAt=", `${updatedAt} (${iso})`);
      expect(spot).to.be.gt(0n);
    } catch (error) {
